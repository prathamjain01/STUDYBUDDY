    ...
    <>
      <header className="bg-background dark:bg-background border-b border-outline-variant w-full h-16 sticky top-0 z-50">
        <div className="flex justify-between items-center px-margin-mobile md:px-margin-desktop max-w-[1200px] mx-auto w-full h-full">
          ...
          <nav className="hidden md:flex gap-lg h-full items-end">
            ...
            <Link className="text-primary border-b-2 border-primary pb-2 font-body-md text-body-md h-full flex items-center mt-[2px]" to="/branches">Branches</Link>
      ...
      <main className="flex-grow relative py-xl px-margin-mobile md:px-margin-desktop max-w-[1200px] mx-auto w-full flex flex-col items-center text-center">
        <h1 className="font-display-lg text-display-lg max-w-3xl mb-md">
          Explore by <span className="text-primary">Branches</span>
        </h1>
        <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mb-xl">
          Find study materials specific to your engineering branch.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter w-full">
            {['Computer Science', 'Information Technology', 'Electronics & Comm.', 'Mechanical Engineering'].map((branch, idx) => (
                <div key={idx} className="group block relative overflow-hidden rounded-xl border border-outline bg-surface-container hover:border-primary transition-colors p-lg flex flex-col items-center justify-center min-h-[160px] cursor-pointer">
                    <h3 className="font-headline-md text-headline-md text-on-background group-hover:text-primary transition-colors text-center">{branch}</h3>
                </div>
            ))}
        </div>
      </main>
